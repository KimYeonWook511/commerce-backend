...
def build(
    ...
    return (
        ...
        "8. 실행 결과와 현재 step 상태/summary 갱신은 실행 상태이며, output/checklist 변경은 로컬 산출물로만 남긴다.\n"
        "9. step 요구사항, Acceptance Criteria, task 문서, root docs, `수정 가능 경로`를 실패 회피 목적으로 임의 수정하지 마라.\n"
        ...
        "12. 실행 output, Acceptance Criteria output, review output, workflow checklist는 로컬 실행 산출물이며 커밋하지 않는다.\n"
    )
