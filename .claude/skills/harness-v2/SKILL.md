...
즉 Stage 6(Execution) "안에서" `execute.py`가 phase의 step들을 순차 실행하는 포함 관계다.
...
- `execute.py` 실행 전 반드시 사용자에게 진행 의사를 확인하고, 사용자가 진행을 승인한 뒤에만 실행한다(가벼운 확인 — 별도 Plan Mode·`ExitPlanMode` 의식은 거치지 않는다). 자동 코드 검증은 없으므로 이 룰은 agent가 직접 지킨다.
...
1. File Drafting 결과(작성한 task 문서·phase 경로)와 실행 계획을 사용자에게 보고하고, 실행 진행 의사를 가볍게 확인받는다. 별도 Plan Mode·`ExitPlanMode` 의식은 거치지 않는다.
...
`execute.py` 실행 직전, `AskUserQuestion`으로 agent별 모델을 한 번에 수집한다. 한 호출에 세 질문을 묶어 전달해 한 화면에 동시에 표시한다.
...
# push는 기본 동작이다. phase 완료 후 원격 push까지 자동 수행한다.
...
# 원격 push를 생략하려면 --no-push (로컬 확인용. PR은 push해야 열 수 있다)
python3 .claude/skills/harness-v2/scripts/execute.py docs/tasks/<task-name>/phases/<phase-name> \
  --developer-model sonnet --reviewer-model opus --commit-model haiku --no-push
```
...
1. `docs/tasks/<task-name>/` 아래에서 step commit agent가 흡수하지 못한 task 문서 잔여 변경분이 있으면 `docs: <task-name> Task 문서 변경분을 반영한다` 커밋으로 묶는다. (`phases/index.json`, `phases/<phase-name>/index.json`은 다음 chore 커밋용이므로 제외)
...
push는 기본 동작이다. 모든 step이 완료되면 현재 feature 브랜치를 원격 저장소로 push한다. `--no-push`를 주면 push를 생략하지만, PR은 원격에 push해야 열 수 있으므로 Stage 7로 진행하려면 push가 필요하다.

#### Stage 6 종료

- Stage 6는 `execute.py`로 phase의 step을 모두 완료하고, phase 끝에서 원격 push한 뒤 PR을 오픈하는 것으로 종료한다.
