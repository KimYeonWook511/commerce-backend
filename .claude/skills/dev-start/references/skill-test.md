...
```bash
python3 .claude/skills/dev-start/scripts/execute.py docs/features/skill-test/phases/0-bootstrap
python3 .claude/skills/dev-start/scripts/execute.py docs/features/skill-test/phases/0-bootstrap --push
```
...
5. 사용자에게 받아야 하는 두 입력을 분리해서 설명한다.
   - 권한 상승 실행 허락
   - 승인 프롬프트 처리 방식: 매번 승인 또는 `prefix_rule=["python3", ".claude/skills/dev-start/scripts/execute.py"]` 저장
...
- 문서 탐색 규칙이 `CLAUDE.md` -> feature 문서 우선 -> 필요 시 루트 문서 추가 순서로 일관된다.
