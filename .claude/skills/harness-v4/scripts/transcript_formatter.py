"""sub-agent transcript(.jsonl)를 사람이 읽는 로그로 변환한다 (harness-v4).
...
def main(argv=None):
    p = argparse.ArgumentParser(description="harness-v4 transcript → human-readable log (증분)")
    ...
    p.add_argument("--role", required=True, help="에이전트 역할/타입 (예: harness-v4-developer)")
