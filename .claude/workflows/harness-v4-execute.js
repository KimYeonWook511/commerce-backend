export const meta = {
  name: 'harness-v4-execute',
  description: 'phase 오케스트레이션 — step별 dev→AC→review→commit→record, 끝에 finalize',
  phases: [
    { title: 'Steps', detail: 'step마다 구현→검증→검토→커밋→기록' },
    { title: 'Finalize', detail: 'completed_at·task index·chore 커밋·push' },
  ],
};

// harness-v4-execute — phase 오케스트레이터 (dynamic workflow)
//
...
function finalizerPrompt(a) {
  return `EXECUTE: ${a.execute}\nPHASE_DIR: ${a.phase_dir}\n`
    ...
    + `phase의 모든 step이 끝났다. finalize로 phase를 마무리하라(completed_at·task index·chore 커밋·push). `
    + `결과 JSON을 그대로 반환하라.`;
}
...
async function runStep(a, step) {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    ...
    const dev = parseAgentJson(await callAgent(
      'harness-v4-developer', devModel, developerPrompt(a, step, attempt, prevError),
    ...
    const rev = parseAgentJson(await callAgent(
      'harness-v4-reviewer', revModel, reviewerPrompt(a, step, attempt, dev),
    ...
    const com = parseAgentJson(await callAgent(
      'harness-v4-committer', comModel, committerPrompt(a, step, attempt, dev),
...
const fin = parseAgentJson(await callAgent(
  'harness-v4-finalizer', null, finalizerPrompt(a), null, 'finalize', 'Finalize')) || {};
