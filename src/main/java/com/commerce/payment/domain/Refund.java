 ...
 *
 * <p>결제와 각자 aggregate 루트라 결제를 식별자로 참조한다. 경계가 실제로 일하는 것은 환불을 만들 때와
 * 성공으로 확정될 때다 — 앞은 한도를, 뒤는 한도의 사용 내역을 바꿔 결제가 알아야 한다. 그 밖의 상태
 * 전이는 두 값 어느 쪽도 바꾸지 않아 결제가 알 필요가 없다. 그래서 자기 낙관 락을 갖고 자기 전이
 * 메서드로 바뀐다 — 결제 버전을 빌려 쓰면 대사가 도는 동안 회원의 환불 요청이 낙관 락 충돌로 밀린다.
 */
@Entity
@Table(
	name = "tbl_refund",
