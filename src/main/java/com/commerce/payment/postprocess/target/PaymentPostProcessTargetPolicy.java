...
public class PaymentPostProcessTargetPolicy {
	...
	// 대사 스캔 윈도우 하한과 단일 출처를 공유하기 위해 public (ReconcilePaymentUseCase 참조)
	...
	// 대사 스캔 윈도우 REQUESTED 하한과 단일 출처를 공유하기 위해 public (ReconcilePaymentUseCase 참조)
	...
	// KEEP_WAITING 판정 후 다음 PG 재조회를 미루는 고정 backoff 간격. 운영 config 승격 전제.
	// null=즉시 대상(미뤄진 적 없음)이므로 NULL 백필 없이 기존 행은 즉시 스캔 대상이 된다.
