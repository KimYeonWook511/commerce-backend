...
public class PaymentRepositoryAdapter implements PaymentRepository {
	 ...
	 * getConstraintName()이 MySQL에서 테이블 prefix를 포함한 형태(tbl_legacy_payment.uk_...)를 반환하므로 전체 이름으로 비교한다.
	...
	private static boolean isApprovedOrderKeyViolation(DataIntegrityViolationException ex) {
		...
		while (cause != null) {
			if (cause instanceof ConstraintViolationException constraintEx) {
				if ("tbl_legacy_payment.uk_payment_approved_order_key".equalsIgnoreCase(constraintEx.getConstraintName())) {
					return true;
				}
			}
