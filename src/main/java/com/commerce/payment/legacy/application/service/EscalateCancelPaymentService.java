...
public class EscalateCancelPaymentService {

	private final PaymentRepository paymentRepository;

	/**
	 * CANCEL 결제 이력을 escalation 표시하는 transition (별도 빈의 public @Transactional).
	 * find → 도메인 가드(escalateCancelPayment) → saveChecked. escalation 대상이 아니면(이미 escalation됐거나 종착) no-op으로 false를 반환한다.
	...
	public boolean escalate(
		...
		if (!payment.escalateCancelPayment(now)) {
			return false;
		}
