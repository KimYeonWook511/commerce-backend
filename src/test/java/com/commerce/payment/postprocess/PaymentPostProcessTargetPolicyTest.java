...
class PaymentPostProcessTargetPolicyTest {
	...
	void resolvePostProcessTarget_whenThresholdNotElapsed_returnNone() {
		...
		failedCancelAttempt.failCancel(PaymentAttemptFailCode.CANCEL_PROCESS_FAILED, "PRE_CANCEL_NOT_COMPLETE",
			now.minusMinutes(1));
		...
		amountMismatchAttempt.approveFail(PaymentAttemptFailCode.AMOUNT_MISMATCH, "AMOUNT_NOT_MATCH",
			now.minusMinutes(1));
	...
	void resolvePostProcessTarget_whenApproveAttemptSucceeded_returnNone() {
		...
		approveAttempt.approveSucceed(now.minusMinutes(1));
	...
	void resolvePostProcessTarget_whenCancelAttemptSucceeded_returnNone() {
		...
		cancelAttempt.cancelSucceed(now.minusMinutes(1));
	...
	private PaymentAttempt failedApproveAttempt(PaymentAttemptFailCode failCode, String failDetail) {
		...
		attempt.approveFail(failCode, failDetail, now.minusMinutes(1));
	...
	private PaymentAttempt failedCancelAttempt(PaymentAttemptFailCode failCode, String failDetail) {
		...
		attempt.failCancel(failCode, failDetail, now.minusMinutes(1));
