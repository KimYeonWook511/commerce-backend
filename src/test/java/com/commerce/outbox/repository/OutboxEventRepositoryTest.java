...
class OutboxEventRepositoryTest {
	...
	@Test
	void findPendingPublishTargetProjections_whenConditionsMatch_returnPendingTargets() {
		// given
	...
	@Test
	void findRetryableFailedPublishTargetProjections_whenRetryTimeReached_returnTargets() {
		// given
	...
	void markPublishingFromPending_whenMatch_updatePublishing() {
		...
		// when
		int updated = outboxEventRepository.markPublishingFromPending(
			target.getId(),
			OutboxEventType.STOCK_RESTORE_REQUESTED
		);
		...
		OutboxEvent updatedEvent = outboxEventRepository.findById(target.getId()).orElseThrow();
	...
	void markPublishingFromRetryableFailed_whenRetryable_updatePublishing() {
		...
		// when
		int updatedRetryable = outboxEventRepository.markPublishingFromRetryableFailed(
			retryable.getId(), OutboxEventType.STOCK_RESTORE_REQUESTED, now);
		int updatedNotRetryable = outboxEventRepository.markPublishingFromRetryableFailed(
		...
		assertThat(outboxEventRepository.findById(retryable.getId()).orElseThrow().getStatus())
			.isEqualTo(OutboxEventStatus.PUBLISHING);
		assertThat(outboxEventRepository.findById(notRetryable.getId()).orElseThrow().getStatus())
	...
	void markSent_whenPublishing_updateSentAndPublishedAt() {
		...
		outboxEventRepository.saveAndFlush(target);
		entityManager.clear();

		// when
		int updated = outboxEventRepository.markSent(target.getId(), OutboxEventType.STOCK_RESTORE_REQUESTED);
	...
	void findStalePublishingTargetIds_whenStaleExists_returnIds() {
		...
		// when
		List<Long> ids = outboxEventRepository.findStalePublishingTargetIds(
			OutboxEventType.STOCK_RESTORE_REQUESTED,
			staleThreshold,
			PageRequest.of(0, 10)
		);
	...
	void recoverStalePublishingEventsByIds_whenMatch_updateOnlyTargets() {
		...
		int updated = outboxEventRepository.recoverStalePublishingEventsByIds(
			OutboxEventType.STOCK_RESTORE_REQUESTED,
		...
		OutboxEvent updatedFirst = outboxEventRepository.findById(first.getId()).orElseThrow();
		OutboxEvent updatedSecond = outboxEventRepository.findById(second.getId()).orElseThrow();
		OutboxEvent unchangedFailed = outboxEventRepository.findById(alreadyFailed.getId()).orElseThrow();
