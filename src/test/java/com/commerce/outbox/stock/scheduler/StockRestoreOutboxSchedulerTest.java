...
class StockRestoreOutboxSchedulerTest {
	...
	void publishPendingEvents_whenResultExists_callServiceAndLog() {
		// given
		given(stockRestoreOutboxService.publishPendingEvents(any()))
		...
		// then
		then(stockRestoreOutboxService).should().publishPendingEvents(any());
	...
	void publishRetryableFailedEvents_whenResultExists_callServiceAndLog() {
		// given
		given(stockRestoreOutboxService.publishRetryableFailedEvents(any()))
			.willReturn(OutboxPublishResult.builder()
				.selectedCount(2)
				.publishedCount(1)
				.failedCount(0)
		...
		// then
		then(stockRestoreOutboxService).should().publishRetryableFailedEvents(any());
		assertThat(listAppender.list)
			.anySatisfy(event -> {
				assertThat(event.getLevel()).isEqualTo(Level.INFO);
				assertThat(event.getFormattedMessage())
	...
	void recoverStalePublishingEvents_whenRecoveredCountPositive_logWarn() {
		// given
		given(stockRestoreOutboxService.recoverStalePublishingEvents(any())).willReturn(5);

		// when
		stockRestoreOutboxScheduler.recoverStalePublishingEvents();

		// then
		then(stockRestoreOutboxService).should().recoverStalePublishingEvents(any());
		assertThat(listAppender.list)
			.anySatisfy(event -> {
	...
	void recoverStalePublishingEvents_whenRecoveredCountZero_notLog() {
		// given
		given(stockRestoreOutboxService.recoverStalePublishingEvents(any())).willReturn(0);
