...
class TraceIdFilterIntegrationTest {
	...
	void request_responseContainsTraceIdHeader() throws Exception {
		mockMvc.perform(get("/__test__/trace"))
			...
			.andExpect(header().exists(MdcKeys.TRACE_ID_HEADER));
	}
	...
	void twoRequests_differentTraceIds() throws Exception {
		...
		String traceId1 = result1.getResponse().getHeader(MdcKeys.TRACE_ID_HEADER);
		String traceId2 = result2.getResponse().getHeader(MdcKeys.TRACE_ID_HEADER);
	...
	void validIncomingHeader_sameValueInResponse() throws Exception {
		mockMvc.perform(get("/__test__/trace")
				.header(MdcKeys.TRACE_ID_HEADER, "existing-trace-123"))
			.andExpect(status().isOk())
			.andExpect(header().string(MdcKeys.TRACE_ID_HEADER, "existing-trace-123"));
	}
	...
	void invalidIncomingHeader_newUuidInResponse() throws Exception {
		MvcResult result = mockMvc.perform(get("/__test__/trace")
				.header(MdcKeys.TRACE_ID_HEADER, "<script>alert(1)</script>"))
			.andReturn();

		String traceId = result.getResponse().getHeader(MdcKeys.TRACE_ID_HEADER);
