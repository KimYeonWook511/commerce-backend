...
### 도메인 예외의 상속 전략 — 무엇으로 골라내느냐로 갈린다

같은 "도메인 예외"라도 상속 전략이 갈린다. 판단축은 도메인(충돌이냐 인프라 장애냐)도, application이 그 예외를 삼키느냐도 아니다. **삼킬 그 예외를 다른 것과 골라낼 수단이 있느냐**다.

| 상황 | 상속 | 이유 |
|---|---|---|
| catch 안 하고 끝단 응답으로 전파 | `CustomException` 류 | 카테고리→상태코드로 **자동 매핑되길 원함**(예: 낙관 락 충돌 409, 인프라 일시 장애 503) |
| catch해서 삼키는데 **오류 코드로 골라낼 수 있다** | `CustomException` 류 | 흐름이 그 코드로 갈라 잡는다. 타입을 새로 만들 이유가 없다 |
| catch해서 삼키는데 **골라낼 수단이 없다** | `RuntimeException` 직접(전용 타입) | 기술 예외가 뭉쳐 오는 자리라 타입 말고는 그 하나를 집을 방법이 없다(예: 유일 제약 위반 중 하나) |

→ `RuntimeException` 직접 상속은 "인프라 장애 한정"도 "catch해서 삼킬 예외 한정"도 아니라 **"골라낼 수단이 그것뿐인 예외 한정"** 규칙이다. 삼킬 예외라도 오류 코드로 갈릴 수 있으면 `CustomException` 류로 둔다 — 새어 나가 자동 매핑되더라도 그 분류가 맞는 답이면 회피할 이유가 없다.
...
- **보상 catch 안 skip은 예외를 안 던지고 흐름을 멈추지 않는다** — 예: 환불 발송 흐름이 "부르기 직전 전이"와 "결과 반영 전이"에서 각각 충돌·가드 위반을 private 메서드로 흡수한다. 충돌은 "다른 주체가 같은 건을 먼저 옮겼다"는 뜻이거나, 그 전이가 루트 행을 함께 저장한다면 "그 루트가 동시에 바뀌어 이번 반영이 밀렸다"는 뜻이다. 어느 쪽이든 돈이 어떻게 됐는지는 대사가 이력으로 확정하므로, 그 자리에서 판정을 강행하지 않고 물러난다.
...
- 그 주문에 UNKNOWN인 결제가 있으면 주문 취소를 `ORDER_REFUND_NOT_AVAILABLE`(409)로 막는다. 판단은 `existsUnknownByOrderId(orderId)`이며, 얼마를 돌려줘야 하는지가 아직 정해지지 않았기 때문이다.
  - **정상 경로에서는 이 차단에 닿지 않는다.** 취소 관문이 결제완료 주문만 통과시키는데, 성공한 결제가 주문 자리를 놓지 않아(아래 항목) 결제완료 주문에는 UNKNOWN이 함께 설 수 없다. 결제 전 주문의 취소는 회원에게 열려 있지 않아 상태 판정에서 먼저 막힌다.
  - 그래도 이 차단을 남긴다. 위 성질이 상태 플래그·자리의 유일 제약·전이 경로에 흩어져 있어 한쪽이 바뀌면 조합이 되살아나고, 그때 이것이 없으면 돌려줄 금액이 정해지지 않은 주문에 환불이 열린다.
