...
차단 기준은 명령 문자열이 아니라 shell token 기준이다. `sudo`, `command`, `env FOO=bar ...`, `git -c key=val ...` 같은 prefix가 있어도 실제 명령이 규칙에 해당하면 차단한다. `&&`, `||`, `;`, `&`, `|`와 **줄바꿈**으로 연결된 복합 명령은 각 명령을 개별 검사하고, 따옴표 안의 구분자는 분리 대상에서 제외한다. 줄바꿈은 셸에서 `;`와 같은 순차 실행 구분자이므로 같게 다룬다. 다만 **값에 해당하는 줄바꿈은 나누지 않는다** — 따옴표 안(여러 줄 commit 메시지)과 here-doc 본문(`<<EOF ... EOF`, 앞 명령의 표준 입력)이 그렇다. here-doc이 끝난 뒤의 명령은 다시 검사 대상이다.
