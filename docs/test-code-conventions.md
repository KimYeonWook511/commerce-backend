...
`sandboxTest` 는 외부 PG sandbox 를 실제로 호출한다. **PG 어댑터나 응답 코드 표를 고쳤을 때** 손으로 돌린다 — 다른 테스트는 결제사를 흉내 낸 응답을 쓰므로, 명세를 잘못 읽었다면 그 흉내도 똑같이 틀린 채로 통과한다.

환경변수가 없을 때의 처리를 두 갈래로 가른다.

- **자격증명·API URL 이 없으면 실패한다.** `sandboxTest` 를 돌린 것은 확인하겠다는 뜻인데 전부 skip 되면 아무것도 안 부르고 빌드가 성공한다. 확인이 안 됐다는 사실을 모르는 것이 가장 나쁘다. 무엇이 없어 못 불렀는지 단언 메시지에 담는다.
- **오래 가지 않는 값(샌드박스 테스트용 결제 번호 등)이 없으면 그 테스트만 skip 한다.** 한 번 쓰면 소모되거나 일정 기간 뒤 사라지므로 비어 있는 것이 정상이다. JUnit `assumeTrue` 로 건너뛰고 리포트에 skipped 로 남긴다. 그런 값을 쓰는 확인은 상설 검증이 아니라 **결제 번호를 새로 만든 뒤 그 유효 기간 안에** 쓰는 도구로 여긴다.

결제 번호를 소모하는 확인(승인·취소를 실제로 태우는 것)은 **명시적 게이트 뒤에** 둔다. 매번 사람이 결제창 인증을 마쳐야 하므로 평소에는 skip 되어야 하고, 돈을 움직이는 호출이라 의도 없이 도는 일이 없어야 한다. 게이트는 환경변수 하나로 두고 기본값을 꺼진 상태로 남긴다.

실패 갈래는 결제 번호를 소모하지 않아 반복 실행이 된다. 매핑에서 위험한 쪽도 실패 코드이므로 그쪽을 두텁게 둔다.
