...
- **되돌리는 쪽에는 적용되지 않는다.** 이미 만들어진 것의 상태를 바꾸는 일은 판정이 읽는 값을 건드리지
  않으므로 그 목적으로 루트를 올릴 이유가 없고, 올리면 배치가 한 바퀴 돌 때마다 회원 요청이 충돌로 밀린다.
- **다만 루트에 다른 값을 남겨야 하는 전이는 있다.** 그때는 판정 방어가 아니라 그 값을 지키려고 루트를
  저장한다(예: 환불 성공 확정이 결제에 실제로 돌아간 금액을 올린다). 그 전이가 사건당 한 번뿐인지 보고,
  되풀이되는 전이라면 위 이유로 올리지 않는다.
- **겉보기에는 잘 돌고 경합에서만 틀리는 종류다.** 단위 테스트도 수동 확인도 통과하고 동시 요청에서만
  드러난다. 그래서 규칙으로 남긴다.

---

### 1.5 목록을 조회한 뒤 건별로 갱신하는 배치는 낙관 락만으로 선점을 가리지 못한다

**낙관 락은 두 갱신이 실제로 겹칠 때만 가른다.** 앞선 갱신이 이미 커밋됐으면, 식별자로 행을 다시 읽는
쪽은 그 최신 버전을 읽어 **충돌 없이 통과한다.**

```
T1: 대상 목록을 조회한다                         (행 X 포함, version 7)
T2: 같은 목록을 조회한다                         (행 X 포함, version 7)
T1: 행 X를 집는다 → 커밋                         version 7 → 8
T2: 행 X를 집으려 식별자로 다시 읽는다           ← version 8 을 읽는다
T2: 저장한다 → 충돌하지 않는다                   version 8 → 9, 같은 건을 두 번 집었다
```

**목록을 도는 시간이 그대로 창이다.** 건마다 외부 호출이 끼면 그 창은 대상 수만큼 길어진다.

- **고를 때 본 값을 함께 넘겨 갱신 직전에 대조한다.** 그 값이 달라졌으면 아무것도 바꾸지 않고 물러난다.
  무엇을 대조할지는 그 배치가 집을 때 올리는 값이다(예: 집은 횟수).
- **낙관 락을 걷지 않는다.** 두 방어가 서로 다른 창을 막는다 — 커밋 전은 낙관 락, 커밋 뒤는 값 대조다.
- **진 쪽이 받는 것이 두 갈래가 된다.** 이미 커밋된 경우는 빈 결과, 겹친 경우는 충돌 예외다. 물러난다는
  결론이 같으면 처리를 나누지 않고 로그만 가른다.
- **읽은 엔티티를 그대로 저장하는 것으로 대신하지 마라.** 그 패턴이 통하는 자리도 있으나, 이 저장소의
  트랜잭션 단위작업은 식별자를 받아 자기 트랜잭션에서 다시 읽는 형태로 통일돼 있다.

---

### 1.6 잠금 뒤 읽기가 잠금 앞 캐시에서 오지 않는다

**행을 잠근 뒤 읽는 값은 언제나 최신 커밋본이다.** 요청 단위 영속성 컨텍스트(`open-in-view`)를 껐기 때문에 성립한다(→ PR#340). 트랜잭션마다 컨텍스트가 새로 열려, 잠금 전에 같은 행을 읽었더라도 그 조회가 캐시를 남기지 못한다.

이 전제가 없으면 다음이 조용히 깨진다.

```
[usecase] 경로를 고르려고 주문을 읽는다        ← 잠금 없음. 1차 캐시에 적재
[tx]      주문 행을 잠근다
          품목을 읽는다                        ← 잠금 앞 캐시에서 온다. 낡은 값
          총량을 판정한다                      ← 이미 다른 요청이 올린 수량이 안 보인다
```

**잠금 대상이 행 하나이고 자식은 지연 로딩으로 읽는 설계에서 특히 그렇다.** 자식 행까지 잠금이 번지지 않게 일부러 그렇게 두므로, 자식이 최신인 것은 "잠금 획득 이후에 처음 읽는다"에만 기댄다. 앞선 조회가 자식을 함께 가져오면(fetch join) 그 순서가 깨진다.

- **순서를 규칙으로 지키지 않고 설정으로 불가능하게 만든다.** 규칙은 새 경로가 어겨도 경합에서만 드러나지만, 설정은 어길 자리가 없다.
- **`open-in-view`는 프로필 공통 설정에 둔다.** 프로필별로 적으면 검증이 도는 프로필만 옛 값으로 남아 운영과 갈리고, 그러면 이 의존을 재현할 수단이 없어진다.
- **검증이 이것을 확인하지는 못한다.** 통합·동시성 검증이 usecase를 직접 불러 요청 경계를 지나지 않으므로, 이 설정은 그 검증에서 켜져 있든 꺼져 있든 작동하지 않는다. 안전의 근거는 지연 로딩 연관을 헤아린 대조이지 검증 통과가 아니다.
- **1.4와 짝이다.** 1.4가 "판정을 통과시킨 쪽이 자기 행을 바꾸는가"를 다룬다면, 여기는 "그 판정이 읽는 값이 최신인가"를 다룬다. 둘 중 하나만 있으면 총량 판정이 새는 창이 남는다.
...
| 정책 | 어디서 | 언제 | 어떻게 |
|------|--------|------|--------|
| **전파(기본)** | 코드 없음 | 클라이언트가 요청 전체를 재시도하면 되는 경우 | **변환 없이** 전파된 DAO 추상 예외를 GlobalExceptionHandler가 받아 → 409 (5.1) |
...
- **결제사 응답을 반영하는 전이** → private skip → 그 사이 다른 주체가 먼저 옮겼으면 그대로 둔다. 루트에 값을 함께 남기는 전이라면 그 루트 행의 충돌도 같은 자리로 오고, 처리도 같다. 돈이 어떻게 됐는지는 대사가 이력으로 확정한다.
