...
2. [트랜잭션 안] 주문 취소 + 환불 의도 생성 + 재고 복구를 한 번에 커밋 (→ PR#305·PR#340)
   - 주문 행을 소유자 조건으로 잠근 뒤, **그 결제·요청자·요청 키의 환불이 이미 있으면** 주문이 내역과 요청 줄을 대조해 같으면 앞 결과를 돌려주고 다르면 거부한다 (→ PR#340)
   - 주문이 **검증·계산**(상태 불변)으로 확정된 줄과 취소 값어치를 내고, 결제가 그 값어치로 환불을 연다. 한도를 판정하고 돌려주기로 한 금액을 올려 결제 버전이 바뀐다
   - 주문이 **반영**한다 — 확정된 줄대로 취소수량을 올리고 내역을 남기며, 모든 잔여가 0일 때에만 취소 상태로 전이한다. 내역이 환불 식별자를 가져야 하는 순환을 검증·계산과 반영을 가르는 것으로 끊는다 (→ PR#340)
   - 환불을 만드는 문이 이 트랜잭션 안에만 있어, 후처리는 "환불 행이 있으면 집행"을 정당성 재확인 없이 믿는다
   - 재고 복구가 맨 뒤다 — 비관 락을 쥔 시간을 짧게 하려고 순서를 고정한다. 부분취소는 **취소한 품목만 요청 수량만큼** 복구하며 상품 식별자 오름차순은 그대로다
3. [트랜잭션 밖, 커밋 뒤] 환불 발송. 못 보내도 취소는 성공이고 발송 배치가 이어받는다
4. 같은 취소 멱등키가 다시 오면 앞서 만든 환불의 결과를 그대로 돌려주고 결제사를 다시 부르지 않는다. 재생 판정 기준은 주문 상태가 아니라 **환불 존재 여부**라 주문이 결제완료로 남아 있어도 같게 동작한다 (→ PR#340)
...
- **`order`** — 주문 생성·취소·만료. 생성은 멱등 키로 중복 방어. 만료는 Spring Batch로 스케줄링하되, 활성 슬롯을 쥔 결제가 걸린 주문은 `BlockingPaymentChecker` port로 만료 대상에서 제외해 만료-대사 경합(돈은 빠졌는데 주문 취소)을 막는다(→ PR#237·PR#305). 생성 tx 내에서 `CartItemRemover` port로 주문된 항목만 cart에서 제거한다.
  - 사용자 취소: **PAID만 받는다**(→ PR#342). 결제 전 주문은 취소 가능 판정에 걸려 거부되고 정리는 만료 배치가 전담한다 — 결제창을 연 채 취소하면 승인 반려와 보상 환불이 정상 흐름에서 열리기 때문이다. 그 판정은 결제완료 취소 관문에 있던 상태 가드를 order aggregate로 옮긴 것이며, 멱등 재생 판정 뒤·승인 결과 미상 검사 앞이라는 호출 위치가 계약이다. **PAID 취소는 품목과 수량을 지정해 일부만 할 수 있다**(→ PR#340) — 주문 품목이 취소수량을 들고 잔여수량을 계산해 답하며, 잔여수량 판정·취소 값어치 계산·상태 전이가 모두 order aggregate 안에서 끝난다. 취소 한 번이 어느 품목 몇 개였는지는 주문 품목의 자식으로 남기고 환불은 루트 식별자로만 가리킨다. 결제 전 취소 관문은 사용자 경로가 닫히면서 호출처를 잃어 제거했고, 결제 전 취소 전이 자체는 만료 배치가 계속 쓴다(→ PR#342). 조율 usecase(tx 없음)가 단위작업 service(tx)를 호출해 한 tx 안에 `환불 의도 생성 + order.cancel() + 재고 복구`를 원자적으로 커밋하고, 커밋 후 best-effort 결제사 환불을 실행한다. 실패·불확실·중단은 환불 발송 배치와 대사에 위임한다(환불 보장은 영속된 의도 + 후처리). 응답에는 이번 환불액과 남은 취소 가능 금액이 함께 나가고, 같은 취소 멱등키가 다시 오면 앞서 만든 환불의 결과를 그대로 돌려준다(→ PR#305). 주문 행은 fetch join 없이 단일 행 락으로 잠가 락 범위를 좁힌다(→ PR#258). order.application이 stock service와 payment의 리포지토리·유스케이스에 의존(기존 order→stock 패턴과 동일).
...
- **`payment`** — 결제 시작·승인 확정·환불·결제사 호출 기록, 그리고 결제·환불 대사. 도메인은 **두 aggregate root**로 갈린다(→ PR#305): `Payment`(승인 생명주기와 그 주문의 활성 슬롯) + `Refund`(결제를 식별자로 참조하고 자기 전이 메서드와 자기 낙관 락을 갖는다). 한 결제에 환불이 여러 건 생길 수 있어 부분환불을 담을 수 있다. 결제 예약(reservation)은 사라졌고 그 역할(신원 확인·결제창 준비물)을 결제 행이 물려받았으며, 레거시 결제 경로와 그 테이블은 제거됐다. 승인·환불 호출은 응답 원본까지 **결제사 호출 기록** 한 곳에 남는다. 결과 불명은 UNKNOWN으로 흔적을 보존하고 그 주문의 새 결제 시작을 `PAYMENT_RESULT_PENDING`(409)로 막는다. 결정 근거는 `docs/adr/`, 예외·충돌 처리는 `docs/exception-strategy.md`·`docs/optimistic-lock-design.md`.
  ...
  - 트랜잭션 경계: 결제사 호출은 항상 트랜잭션 밖이고, **부르기 직전 상태 전이를 따로 커밋한 뒤에** 부른다. 유스케이스는 흐름만 조립하고 트랜잭션을 열지 않는다. 승인 결과가 결제 도메인 밖(주문)까지 바꾸는 자리만 전용 서비스로 갈라 두고, 결제·환불 하나의 전이는 각 도메인의 한 클래스에 모은다(환불 쪽은 성공 확정만 결제를 함께 갱신한다). 그 메서드들은 다른 트랜잭션 서비스를 부르지 않고 리포지토리와 도메인 객체를 직접 다룬다.
  - 락: 결제와 환불이 각자 낙관 락을 갖고, 가르는 기준은 **결제가 든 금액이 바뀌는가**다. 환불을 만들면 돌려주기로 한 금액이, 그 환불이 성공으로 확정되면 실제로 돌아간 금액이 올라 결제 버전이 바뀐다. 앞엣것이 동시 요청 방어다(진 쪽은 환불까지 함께 롤백된다). 그 둘 말고 환불 상태 전이는 결제 버전을 올리지 않는다 — 올리면 대사가 도는 동안 회원의 환불 요청이 충돌로 밀린다. 확정은 환불 건당 한 번뿐이라 그 해악이 되풀이되지 않는다.
  ...
  - 대사: `@Scheduled` 진입점(presentation/scheduler)이 결제·환불 대사와 통지·만료·발송을 깨운다. 결과를 모르는 건은 결제사 **이력 조회**로 확정하고(재요청 아님, 이중과금 방지), 회수를 멈추지 않는다 — 멈추면 환불은 돈이 안 돌아간 채 한도가 막히고, 승인은 활성 슬롯을 쥔 채 남아 그 주문을 영영 결제할 수 없다. 한 주기의 대상은 개수로 자르지 않고 다 처리하며, 대상이 임계를 넘으면 밀렸다는 것을 알린다 — 자르면 남은 건이 다음 주기로 밀릴 뿐이고 얼마나 밀렸는지가 아무 데도 남지 않는다. 집었다는 기록은 결제사를 부르기 전에 따로 커밋하고, 다른 주기가 이미 집었는지는 집을 때 본 회차를 다시 확인하는 것과 낙관 락이 함께 가린다 — 앞선 갱신이 커밋된 뒤와 커밋되기 전이라는 서로 다른 창을 각자 막는다. 운영자 통지는 `NotificationPort`로 나가고 보낸 뒤에 시각을 남긴다. 이 진입점들은 결제 후처리 전용 스레드 풀에서 돈다 — 이 도메인만 결제사를 부르므로, 공용 풀에 두면 결제사가 느려질 때 주문 만료와 재고 복구까지 함께 멈춘다. 그 안에서 대사 둘만 다시 떼어 대사 전용 풀에서 돈다 — 대사는 한 주기에 대상을 다 처리해 밀린 만큼 스레드를 오래 잡고, 같은 풀에 두면 그동안 환불 발송이 스레드를 얻지 못해 접수된 환불이 안 나간다.
