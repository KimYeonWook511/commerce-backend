...
- `payment` core는 결제 준비·완료 반영·시도 이력 관리를 담당한다. `naverpay`는 provider 서브패키지로, PG 호출과 내부 결제 상태 반영을 분리한다. `Payment` 는 `orderId: Long` 으로 Order 를 ID 참조한다(ADR-020). Payment ↔ Order 는 별 aggregate 다 (cascade / orphanRemoval 없음, lifecycle 결합 약함). `PaymentAttempt` 는 `merchantPayKey` / `pgPaymentId` / `provider` 식별자 기반이며 다른 entity 와 객체 참조가 없다. Payment 도메인은 fetch join 사용처가 없으므로 fetch join 대체 원칙은 `docs/tasks/order-jpa-association-decouple/adr.md` 에서 단일 관리한다. 세부 결정은 `docs/tasks/payment-jpa-association-decouple/adr.md` 참조. ADR-020 후속 트랙 series (Stock / Order / Payment) 완료. 후속 DB FK 일괄 제거는 별도 트랙.
