...
이 문서는 제품이 제공하는 기능의 상위 인덱스다. 각 기능의 상세 계약은 `docs/api-spec.md`, 설계 결정은 `docs/adr/`의 결정별 파일, 기능별 작업 맥락은 각 task의 `docs/tasks/<task>/prd.md`를 가리킨다. 본 문서는 task PRD 본문을 흡수하지 않는다.
...
| --- | --- | --- | --- |
| 인증 | 회원가입·로그인·토큰 재발급. JWT(access) + Redis(refresh). Redis 장애 시 strict 실패 | api-spec §인증 (→ PR#59·PR#97) | `auth-redis-timing`, `auth-refresh-token-store-unavailable` |
...
| 재고 | 주문 경로 차감·복구, 관리자 초기 생성·수동 조정, 변경 이력 | api-spec §관리자 재고 (→ PR#59·PR#68) | `stock-management` |
| 장바구니 | 담기(UPSERT)·조회(최신가 재조립·구매불가 마킹)·수량 변경·삭제 | api-spec §장바구니 (→ PR#166) | `cart` |
| 주문 | 생성·취소·만료 배치. `Idempotency-Key` 멱등 (Redis in-flight + DB unique). PAID 주문 취소 시 전액 환불·재고 복구(환불 의도 단일 tx 영속화 + CANCEL 대사 보장) | api-spec §주문 (→ PR#59·PR#180·PR#258) | `order-idempotency`, `order-idempotency-cache-simplification`, `paid-order-cancel-refund` |
| 결제 | 외부 PG(네이버페이) 예약(reserve)·승인(approve). 두 테이블 분리(Reservation + Payment append-only), UNKNOWN 마킹, 보상, 진입·예약 동시성 이중청구 가드 | api-spec §결제 (→ PR#205·PR#101·PR#125·PR#235) | `payment-order-redesign`, `payment-attempt-idempotency`, `payment-attempt-service-split`, `payment-attempt-state-transition-policy`, `payment-compensation-policy`, `payment-compensation-to-domain`, `approval-concurrency-guard` |
