...
설명:
- 회원이 자신의 주문을 취소합니다. **`PAID`(결제 완료) 주문만 취소할 수 있습니다.**
- `INIT`(결제 전) 주문은 취소할 수 없습니다. 그 주문에 걸린 결제의 유무·상태와 무관하게 취소 불가로 거부하며, 정리는 주문 만료 배치가 맡습니다.
- `PAID` 취소: **품목과 수량을 지정해 일부만 취소할 수 있습니다.** 지정한 품목의 취소수량이 오르고, 그 값어치만큼 환불이 열리며, 취소한 품목만 요청 수량만큼 재고가 복구됩니다. 모든 품목의 잔여수량이 0이 될 때에만 주문이 `CANCELED`로 전이합니다. 환불 의도는 취소와 단일 tx로 영속화되고, 실제 PG 환불은 best-effort로 실행되어 실패·불확실·중단은 환불 대사가 마무리합니다.
- **품목 목록을 생략하면 잔여가 남은 품목의 잔여 전부**를 취소합니다. 전액취소는 부분취소의 특수한 경우이며 같은 주소·같은 경로를 지납니다.
- 중복 요청 방지를 위해 `Idempotency-Key` 헤더가 필요합니다.
...
- Body (선택 — 없으면 잔여 전부 취소)
  - `items`: 취소할 품목 목록. 없거나 비면 잔여가 남은 품목의 잔여 전부로 봅니다.
    - `items[].orderItemId`: 주문 품목 ID. 필수, 양수.
    - `items[].quantity`: 취소 수량. 필수, 1 이상.
...
같은 `orderItemId`가 여러 줄로 오면 합치지 않고 거부합니다. 취소는 되돌릴 수 없어 모호한 의도를 서버가 해석해 실행하지 않습니다.
...
응답:
- `refundStatus`: 환불 진행 상태. `COMPLETED` / `IN_PROGRESS` 둘뿐이며, 내부 환불 상태 다섯이 아래처럼 접힙니다.

  | 내부 환불 상태 | `refundStatus` |
  | --- | --- |
  | `SUCCEEDED` | `COMPLETED` |
  | `READY` · `IN_PROGRESS` · `UNKNOWN` · `MANUAL_REVIEW` | `IN_PROGRESS` |

- `refundedAmount`: 이번 요청으로 환불되는 금액 — **이번에 취소한 품목의 값어치**(`Σ(주문 시점 단가 × 취소 수량)`)입니다. 주문 총액도, 그 결제에 열린 환불 금액의 합도 아닙니다.
- `remainingAmount`: 앞으로 더 취소할 수 있는 금액(`승인 금액 − 돌려주기로 한 금액`). 취소 가능 여부 판정은 이 금액이 아니라 **품목별 잔여수량**이 합니다 — 두 값이 답하는 질문이 다릅니다. 결과를 모르는 환불도 한도를 이미 잡고 있어 여기서 빠지므로, "아직 안 돌아온 돈"이 아니라 취소 가능 금액입니다.
...
같은 `Idempotency-Key`로 다시 요청하면 새 환불을 만들지 않고 앞서 만든 환불의 결과를 그대로 응답합니다(실패가 아닙니다). 결제사를 다시 부르지도 않습니다. 재생 여부는 **주문 상태가 아니라 그 결제·요청자·요청 키의 환불이 이미 있는가**로 판정하므로, 부분취소로 주문이 `PAID`로 남아 있어도 같게 동작합니다.

**같은 키에 내용이 다르면 거부합니다.** 대조는 품목 식별자와 수량의 조합으로 하며 목록의 순서는 따지지 않습니다. 요청 줄이 비어 있으면 대조하지 않고 앞 결과를 돌려줍니다 — 보낸 목록이 없으니 비교할 내용도 없고, 부분취소 도입 이전에 만들어진 환불에는 품목 내역이 없어 이 면제가 그 재요청을 지킵니다.

실패:
- 본인 주문이 아니거나 없으면 주문을 찾을 수 없음으로 응답합니다.
- 취소 불가 상태(`INIT`, 이미 CANCELED 등)는 거부합니다. 같은 요청 키로 만들어진 환불이 이미 있으면 상태를 보기 전에 그 결과를 그대로 돌려주므로, 취소로 종착한 주문의 재요청은 이 거부에 걸리지 않습니다.
- 승인 결과를 모르는 결제가 걸려 있으면 환불 불가로 거부하며, 환불 대상(성공한 승인)이 없는 정합성 오류도 거부합니다(취소를 강행하지 않음). 다만 성공한 결제가 주문 자리를 놓지 않아 `PAID` 주문에는 결과 미상 결제가 함께 설 수 없으므로, 이 거부는 그 성질이 흔들릴 때를 받치는 안전망입니다.
...
- `400 INVALID_REQUEST`: 그 주문에 없는 `orderItemId`, 같은 `orderItemId`가 여러 줄, `quantity`가 1 미만, 목록에 `null` 원소. 남의 주문 품목인지 없는 품목인지 구분해 알려주지 않습니다.
- `409`: 잔여수량을 넘는 취소. 한 품목이라도 넘으면 요청 전체가 거부되고 아무것도 바뀌지 않습니다.
- `409`: 취소할 잔여가 하나도 없음.
- `409 ORDER_CANCEL_IN_PROGRESS`: 같은 `Idempotency-Key`로 다른 요청이 처리 중. 클라이언트는 backoff 후 재시도 권장.
- `409 OPTIMISTIC_LOCK_CONFLICT`: 앞선 환불의 확정과 겹쳐 결제 행 경합이 났습니다. 아무 상태도 바뀌지 않으며 다시 시도하면 됩니다. 같은 키가 처리 중인 것과는 다른 실패입니다 — 이쪽은 서로 다른 요청이 같은 결제 행을 다툰 것입니다.
