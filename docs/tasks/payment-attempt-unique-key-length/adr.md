...
- 동시성 테스트의 race 발생 자체 가시화는 포기한다. PR 리뷰(codex P2)에서 `errors.anyMatch(DataIntegrityViolationException)` 단언이 환경 의존적(빠른 환경에서는 race가 거의 항상 발생하나, CPU 코어가 적거나 CI 부하가 큰 환경에서는 첫 thread가 commit한 뒤 나머지가 사전 find 분기로 빠져 race가 발생하지 않을 수 있음)이라는 지적이 있었다. latch barrier로 race를 강제하는 시도도 했으나 `@MockitoSpyBean` + Spring CGLIB proxy의 이중 wrapping이 `UndeclaredThrowableException`을 만들어 실패. 결국 anyMatch 단언을 제거하고 `countAttempts == 1` 데이터 invariant + `assertRaceOrPaymentError` helper 조합으로 안전망을 검증한다. 잃는 것은 race 발생 자체의 가시화이고, schema 회귀 검출(count)과 발생 예외 분류(helper)는 환경 독립적으로 유지된다.
- `NaverPayServiceConcurrencyTest`는 20 thread + 보상 흐름의 connection 수용을 위해 클래스 단위 HikariCP 설정(`maximum-pool-size=30`, `minimum-idle=10`, `connection-timeout=30000`)을 명시한다. 다른 concurrency 테스트(`OrderConcurrencyServiceTest` 등)의 컨벤션과 같은 방식.
