# Step 1: split-compensation-by-failure-meaning
...
- `src/main/java/com/commerce/payment/naverpay/application/NaverPayApprovalService.java` — step 0에서 변경된 파일
...
- `failApproveAndCancelApprovedPayment` 내부 로직을 이 step에서 변경하지 마라. 이유: step 0에서 이미 Payment 체크 로직이 추가됐으며, catch 분기 분리와는 관심사가 다르다.
