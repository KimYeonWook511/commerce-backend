# Step 0: race-priority-by-payment-existence
...
- `completeVerifiedApproval`의 catch 분기 구조를 이 step에서 변경하지 마라. 이유: step 1에서 분리 처리하므로 두 변경을 혼재하면 각 step의 독립 검증이 불가능해진다.
