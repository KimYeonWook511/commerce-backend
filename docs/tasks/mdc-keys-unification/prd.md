...
- MDC 키·관련 상수(MDC 키, traceId 헤더 이름, traceId 유효성 정규식)를 단일 `MdcKeys` 클래스로 통합해 정의 위치를 일원화한다.
- 모든 사용처가 `MdcKeys`를 단일 출처로 참조하게 만든다.
- 동작과 외부 계약은 변경하지 않는다. 본 태스크는 식별자·기호 정리만 수행한다.

## 범위

### 포함 범위

- 신규 `src/main/java/com/commerce/common/log/MdcKeys.java`
- main 소스 5개 갱신:
  - `src/main/java/com/commerce/common/log/filter/TraceIdFilter.java`
  - `src/main/java/com/commerce/common/log/kafka/TraceIdKafkaProducerInterceptor.java`
  - `src/main/java/com/commerce/common/log/kafka/TraceIdRecordInterceptor.java`
  ...
  - `src/main/java/com/commerce/security/filter/JwtAuthenticationFilter.java` — `AccessLogFilter.MEMBER_ID_MDC_KEY` 참조를 `MdcKeys.MEMBER_ID`로 교체. `AccessLogFilter.MEMBER_ID_ATTRIBUTE` 참조는 유지.
...
- 루트 문서 갱신: `docs/logging-conventions.md` §8에 "MDC 키는 `MdcKeys`에서 단일 관리한다" 명시 추가.
...
- `MEMBER_ID_ATTRIBUTE`(HTTP request attribute) — MDC 키가 아니므로 `MdcKeys`로 이동하지 않는다.
- `orderId`, `paymentId` 등 도메인 식별자 MDC 키 — 본 태스크에서 신규 도입하지 않는다. 도메인 도입 시점에 `MdcKeys`에 추가한다.

## 주요 시나리오

- HTTP 요청이 들어오면 `TraceIdFilter`가 `MdcKeys.TRACE_ID_HEADER`로 요청 헤더를 읽고 `MdcKeys.TRACE_ID`로 MDC에 push한 뒤 동일 헤더로 응답에 set한다. 동작과 응답 헤더 이름은 변경 전과 동일하다.
- 인증된 요청은 `JwtAuthenticationFilter`가 `MdcKeys.MEMBER_ID`로 MDC에 push한다. 동작과 MDC 키 문자열은 변경 전과 동일하다.
- Kafka producer는 메시지 발행 시 `MdcKeys.TRACE_ID_HEADER` 헤더에 MDC `MdcKeys.TRACE_ID` 값(또는 신규 UUID)을 부착한다. 동작은 변경 전과 동일하다.
- Kafka consumer는 동일 헤더에서 traceId를 추출해 `MdcKeys.TRACE_ID`로 MDC에 push하고, `afterRecord()`에서 `MDC.remove(MdcKeys.TRACE_ID)`로 정리한다. 동작은 변경 전과 동일하다.

## 요구사항

- 신규 `MdcKeys` 클래스를 `com.commerce.common.log` 패키지에 둔다.
- `MdcKeys`는 `public final class` + `private` 기본 생성자로 인스턴스화를 차단한다.
- 노출 상수는 다음 4개로 한정한다.
  - `public static final String TRACE_ID = "traceId"`
  - `public static final String MEMBER_ID = "memberId"`
  - `public static final String TRACE_ID_HEADER = "X-Trace-Id"`
  - `public static final java.util.regex.Pattern VALID_TRACE_ID = Pattern.compile("^[A-Za-z0-9_-]{1,64}$")`
- 사용처는 모두 `MdcKeys`를 import해 참조한다. 클래스 내부 `private/static final` 중복 상수와 직접 리터럴은 제거한다(예외: logback 패턴, `@DisplayName` 표시 텍스트).
- 외부에 노출되는 MDC 키 문자열, HTTP 응답 헤더 이름, 정규식 패턴 값은 변경하지 않는다.
- `MEMBER_ID_ATTRIBUTE`는 `AccessLogFilter`에 유지한다.

## 제약사항

- 동작 변화 없는 refactor 작업이다. Filter/Interceptor의 흐름·예외 처리·`MDC.remove` 호출 위치는 그대로 유지한다.
- `MDC.clear()` 도입 금지 — 기존 컨벤션대로 키 단위 `MDC.remove(MdcKeys.XXX)`만 사용한다.
- `MdcKeys.VALID_TRACE_ID`는 `Pattern` 객체 자체를 단일 정의로 노출한다. 호출부에서 `Pattern.compile(...)`을 반복 호출하지 않는다.
- 본 태스크 안에서는 `MdcKeys` 외 별도 상수 클래스(`TraceIdConstants` 등) 신설을 금한다.
