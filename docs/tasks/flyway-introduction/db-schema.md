...
엔티티 12개 기준 현 스키마를 **`V1__init.sql`로 베이스라인화**한다. 신규 테이블/컬럼 추가는 없다. 이번 작업의 핵심은 *어떤 스키마가 들어가느냐*가 아니라 *그 스키마가 Flyway 마이그레이션 스크립트로 명시적 관리 대상이 되느냐*이다.
...
기존 엔티티에 정의된 인덱스가 V1에 모두 포함되는지 검증한다.

- `tbl_outbox_event`: `idx_outbox_event_type_status_next_retry_id`
- `tbl_processed_event`: `idx_processed_event_event_id_consumer_type`
- `tbl_member`: `email` UNIQUE
- `tbl_order`: `uk_order_member_idempotency`
...
- `tbl_stock`: `product_id` UNIQUE (1:1 관계)

## 데이터 무결성

- 모든 PK: `BIGINT AUTO_INCREMENT` (`GenerationType.IDENTITY`)
- 모든 ENUM 컬럼: `VARCHAR(...)` (ADR-018 / `@JdbcTypeCode(SqlTypes.VARCHAR)`)
- multi-column unique constraint 컬럼: `@Column(length=...)` 명시 → V1 dump에서 InnoDB 한도 검증 (ADR-023)
- `@Version` 컬럼(`tbl_order`, `tbl_stock`, `tbl_cart_item`): `NOT NULL`
- `BaseTimeEntity` 상속 컬럼(`created_at`, `updated_at`): 모든 도메인 테이블에 존재
