...
- 태스크 진행 중 상세 기획과 설계는 `docs/tasks/<task-name>/`에서 관리한다.
- 태스크가 완료되면 필요한 내용을 루트 `docs/` 문서에 반영한다.
- 태스크 폴더 문서는 삭제하지 않고 유지하며, 이후 변경의 근거 문서로 사용한다.
...
docs/
└── tasks/
    ├── README.md                    # 태스크 문서 운영 가이드
    ├── _templates/                  # 태스크 문서 기본 템플릿
    │   ├── prd.md
    │   ├── architecture.md
    │   ├── adr.md
    │   ├── api-spec.md
    ...
    └── <task-name>/                 # 개별 태스크 작업 루트
        ├── prd.md                   # 태스크 요구사항
        ├── architecture.md          # 태스크 구조 설계
        ├── adr.md                   # 태스크별 설계 결정
        ├── api-spec.md              # 태스크 API 계약
        ├── db-schema.md             # 태스크 DB 변경 사항
        └── phases/
            ├── index.json           # 태스크 내부 phase 목록
            └── <phase-name>/
...
- `docs/tasks/<task-name>/`는 해당 태스크의 작업 루트다.
- `prd.md`, `architecture.md`, `adr.md`, `api-spec.md`, `db-schema.md`는 태스크 기획과 설계 문서다.
...
새 태스크 폴더를 만들 때는 아래 문서를 기본 생성한다.
...
태스크 문서는 `docs/tasks/_templates/` 아래 템플릿을 복사해 시작한다.
workflow checklist는 `docs/tasks/_templates/phases/workflow-checklist.json`을 복사해 시작한다.

## 태스크별 `phases`

- 태스크별 step 실행 상태는 `docs/tasks/<task-name>/phases/` 아래에서 관리한다.
- `docs/tasks/<task-name>/phases/index.json`은 해당 태스크 내부의 phase 목록만 관리한다.
- 각 phase 상세 상태는 `docs/tasks/<task-name>/phases/<phase-name>/index.json`에 둔다.
...
태스크 작업 시 agent는 아래 순서로 문서를 읽는다.

1. `CLAUDE.md` 또는 `AGENTS.md` (사용 중인 도구 기준)
2. `docs/tasks/<task-name>/` 아래 태스크 문서
3. 해당 태스크의 `phases` 문서와 step 문서
...
태스크의 **PR이 머지된 시점부터** 해당 task 폴더 문서는 불변으로 다룬다. 이후 같은 영역의 코드/정책에 변경이 생겨도 이 task 폴더 본문은 수정하지 않는다.

### 완료의 정의

- **완료**: 해당 task의 PR이 머지된 상태. 머지 시점부터 그 task 폴더 문서 전체가 불변.
- **진행 중**: 해당 task의 PR이 머지 전인 상태. 단 "진행 중"의 범위는 **현재 worktree에서 본인이 작업 대상으로 삼은 바로 그 task 한 개에 한정**한다.
  - 그 진행 중 task 폴더 내부 문서는 작업 흐름에 따라 자유 수정 OK. 리뷰 코멘트 반영, 설계 변경, 단계 갱신 등 머지 전까지 발생하는 모든 변경을 본문에 반영해도 된다.
  - **다른 task 폴더는 어떤 경우에도 수정하지 않는다.** 여기엔 (1) 이미 머지 완료된 task, (2) 현재 작업 대상이 아닌 다른 진행 중 task (다른 worktree/브랜치에서 진행되는 task) 가 모두 포함된다. 다른 task의 코드/정책이 이번 작업과 관련 있어 보여도 그 task 폴더 본문은 손대지 않는다.
